...
const Annotations = () => {
  ...
  const { annotations, isLoading, getAnnotations, addAnnotation } = useAnnotations();
  ...
  return (
    isLoading && !window['player'] ? <h1>Waiting request</h1> :
    ...
    <article className="mainDiv vh-50 br2 ba dark-gray b--black-10 mh1 bn">
      ...
      .map((annotation: IAnnotation, i: number) =>
         (<div className="mv2" key={i}>
          ...
          <span className="f6 f5-ns lh-copy measure baskerville ph1 mid-gray underline-hover pointer"
      ...
      {annotations?.length > perPage && <Pagination
      ...
      <Formik initialValues={{} as any} onSubmit={data => submitAnnotation(data)}>
        <Form>
          <Field
           InputProps={{
